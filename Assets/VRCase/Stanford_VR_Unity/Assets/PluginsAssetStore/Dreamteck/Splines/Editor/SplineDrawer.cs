...
namespace Dreamteck.Splines {
    ...
    public static class SplineDrawer
    {
        ...
        public static void IncludeComputerInDrawing(SplineComputer comp)
        {
            if (drawComputers.Contains(comp)) return;
            drawComputers.Add(comp);
        }

        public static void ExcludeComputerFromDrawing(SplineComputer comp)
        {
            for(int i = 0; i < drawComputers.Count; i++)
            {
        ...
        public static void DrawSplineComputer(SplineComputer comp, double fromPercent = 0.0, double toPercent = 1.0, float alpha = 1f)
        {
            ...
            if (!comp.drawThinckness)
            {
                ...
                while (true)
                {
                    ...
                    toPoint = comp.EvaluatePosition(percent);
            ...
            else
            {
                ...
                if (comp.billboardThickness) fromNormal = (editorCamera.transform.position - from.position).normalized;
                ...
                while (true)
                {
                    ...
                    if (comp.billboardThickness) toNormal = (editorCamera.transform.position - to.position).normalized;
