    ...
    public class Spline {
       ...
       public int iterations
        ...
        public Vector3 EvaluatePosition(double percent)
        {
            ...
            return GetPoint(doubleIndex - pointIndex, pointIndex);
        }
        ...
        /// <returns></returns>
        public void Evaluate(SplineResult[] samples, double from = 0.0, double to = 1.0)
        {
            if (points.Length == 0) {
        ...
        private double GetClosestPoint(Vector3 point, double from, double to, int steps)
        {
            ...
            for (int i = 0; i <= steps; i++)
            {
                ...
                float L = (pos - point).sqrMagnitude;
                if (L < Ref)
                {
                    Ref = L;
                    Res = t;
        ...
        private Vector3 GetPoint(double percent, int pointIndex)
        {
            switch (type)
            {
                ...
                case Type.BSpline: return BSPGetPoint(percent, pointIndex);
        ...
        private Vector3 LinearGetPoint(double t, int i)
        {
            ...
            if (i + 1 < points.Length)
            {
                ...
                return Vector3.Lerp(points[i].position, points[i + 1].position, (float)t); 
            } else
            {
                if (i < points.Length) return points[i].position;
        ...
        private Vector3[] GetPCFFPoints(int i)
        {
