    ...
    public class TubeGenerator : MeshGenerator
    {
        ...
        void Generate()
        {
            ...
            for (int i = 0; i < clippedSamples.Length; i++)
            {
                ...
                if (i > 0 && (uvMode == UVMode.UniformClip || uvMode == UVMode.UniformClamp)) uvs.y += Vector3.Distance(clippedSamples[i].position, clippedSamples[i - 1].position);
                else uvs.y = (float)clippedSamples[i].percent;
                for (int n = 0; n < _sides + 1; n++)
                {
                    ...
                    uvs.x = 1f - anglePercent;
                    tsMesh.uv[vertexIndex] = GetUVs();
        ...
        void GenerateFlatCaps()
        {
            ...
            for (int i = 0; i < _sides + 1; i++)
            {
                ...
                tsMesh.uv[index] = tsMesh.uv[i] = Quaternion.AngleAxis(_integrity * ((float)(bodyIndex) / (_sides - 1)), Vector3.forward) * Vector2.right * 0.5f * capUVScale + Vector3.right * 0.5f + Vector3.up * 0.5f;
            }
        ...
        void GenerateRoundCaps()
        {
            ...
            float lastUvsy = uvs.y;
            uvs.y = -1f / _roundCapLatitude * size * 0.5f;
            for (int lat = 1; lat < _roundCapLatitude+1; lat++)
            {
                float latAngle = 90f * ((float)lat / _roundCapLatitude);
                if ((uvMode == UVMode.UniformClip || uvMode == UVMode.UniformClamp)) uvs.y -= 1f / _roundCapLatitude * size * 0.5f;
                else uvs.y = -((float)lat / _roundCapLatitude) * _capUVScale;
                for (int lon = 0; lon <= sides; lon++)
                {
                    ...
                    tsMesh.vertices[index] = center + lookRot * rot * -Vector3.right * size * 0.5f;
                    tsMesh.normals[index] = (tsMesh.vertices[index] - center).normalized;
                    uvs.x = 1f - anglePercent;
                    tsMesh.uv[index] = GetUVs();
                }
            }
            ...
            for (int lat = 1; lat < _roundCapLatitude+1; lat++)
            {
                float latAngle = 90f * ((float)lat / _roundCapLatitude);
                if (uvMode == UVMode.UniformClip || uvMode == UVMode.UniformClamp) uvs.y += 1f / _roundCapLatitude * size * 0.5f;
                else uvs.y = (float)clippedSamples[clippedSamples.Length - 1].percent + ((float)lat / _roundCapLatitude) * _capUVScale;
                for (int lon = 0; lon <= sides; lon++)
                {
                    float anglePercent = (float)lon / sides;
