...
namespace Dreamteck.Splines {
    ...
    public class SplineUser : MonoBehaviour {
        ...
        public void Project(SplineResult result, Vector3 point, double from = 0.0, double to = 1.0)
        {
            if (samples.Length == 0) return;
            if (samples.Length == 1)
            {
        ...
        public float CalculateLength(double from = 0.0, double to = 1.0) {
            float length = 0f;
            int startIndex = GetSampleIndex(from);
            int endIndex = GetSampleIndex(to);
            Vector3 current = samples[startIndex].position;
            for (int i = startIndex + 1; i <= endIndex; i++)
            {
                length += (samples[i].position - current).magnitude;
                current = samples[i].position;
            }
            float fromDistance = Vector3.Distance(EvaluatePosition(from), samples[startIndex].position);
            if (endIndex < samples.Length - 1) endIndex++;
            float toDistance = Vector3.Distance(EvaluatePosition(to), samples[endIndex].position);
            length -= (fromDistance + toDistance);
            return length;
        }
