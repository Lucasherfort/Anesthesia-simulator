...
namespace Dreamteck.Splines
{
    ...
    public class ObjectBenderEditor : SplineUserEditor
    {
        ...
        void PropertyEditor(ObjectBender.BendProperty[] properties)
        {
            ...
            GUILayout.Box("", GUILayout.Height(85), GUILayout.Width(EditorGUIUtility.currentViewWidth - 50));
            Rect lastRect = GUILayoutUtility.GetLastRect();
            lastRect.height = 18;
            GUI.BeginGroup(lastRect);

            if(selected.Count == 1) GUI.Label(new Rect(5f, 1f, EditorGUIUtility.currentViewWidth * 0.7f, 22), properties[selected[0]].transform.transform.name);
            else GUI.Label(new Rect(5f, 1f, EditorGUIUtility.currentViewWidth * 0.7f, 22), "Multiple objects");
            EditorGUI.BeginChangeCheck();
            enabled = EditorGUI.Toggle(new Rect(EditorGUIUtility.currentViewWidth - 50f - 22f, 1f, 22, 22), enabled);
            GUI.EndGroup();
            GUI.BeginGroup(GUILayoutUtility.GetLastRect());
            applyRotation = EditorGUI.Toggle(new Rect(5, 22, EditorGUIUtility.currentViewWidth / 2f - 30, 20), "Apply rotation", applyRotation);
            applyScale = EditorGUI.Toggle(new Rect(5, 40, EditorGUIUtility.currentViewWidth / 2f - 30, 20), "Apply scale", applyScale);
            if (showSpline) bendSpline = EditorGUI.Toggle(new Rect(5, 58, EditorGUIUtility.currentViewWidth / 2f - 30, 20), "Bend Spline", bendSpline);
            if (showMesh)
            {
                bendMesh = EditorGUI.Toggle(new Rect(EditorGUIUtility.currentViewWidth / 2f - 25, 22, EditorGUIUtility.currentViewWidth / 2f - 30, 20), "Bend Mesh", bendMesh);
                if (bendMesh)
                {
                    if (showCollider)
                    {
                        bendCollider = EditorGUI.Toggle(new Rect(EditorGUIUtility.currentViewWidth / 2f - 25, 40, EditorGUIUtility.currentViewWidth / 2f - 30, 20), "Bend Collider", bendCollider);
                        if (bendCollider)
                        {
                            colliderUpdateRate = EditorGUI.FloatField(new Rect(EditorGUIUtility.currentViewWidth / 2f - 25, 58, EditorGUIUtility.currentViewWidth / 2f - 30, 18), "  Update Rate", colliderUpdateRate);
                        }
                    }
            ...
            } else GUI.Label(new Rect(EditorGUIUtility.currentViewWidth / 2f - 25, 22, EditorGUIUtility.currentViewWidth / 2f - 30, 22), "No Meshes Available");
            GUI.EndGroup();
        ...
        public override void BaseGUI()
        {
           ...
           if (bender.autoNormals) bender.upVector = EditorGUILayout.Vector3Field("Up Vector3", bender.upVector);
            if (!bender.bend)
            {
                float scrollHeight = Mathf.Min(bender.bendProperties.Length, 10) * 22;
                if (selected.Count > 0) scrollHeight -= 90;
                ...
                for (int i = 0; i < bender.bendProperties.Length; i++)
                {
                    ...
                    if (!bender.bendProperties[i].enabled)
                    {
                        ...
                        if (isSelected) GUI.color = Color.Lerp(Color.gray, SplineEditorGUI.selectionColor, 0.5f);
                    }
                    else
                    {
                        if (isSelected) GUI.color = SplineEditorGUI.selectionColor;
