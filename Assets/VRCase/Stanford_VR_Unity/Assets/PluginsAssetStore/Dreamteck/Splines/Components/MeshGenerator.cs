...
namespace Dreamteck.Splines
{
    ...
    public class MeshGenerator : SplineUser
    {
        ...
        protected Vector2 uvs = Vector2.zero;
        protected float uvBeginDistance = 0f;
        ...
        {
            if (uvMode == UVMode.UniformClip) uvBeginDistance = CalculateLength(0.0, clippedSamples[0].percent);
        ...
        protected Vector2 GetUVs()
        {
            switch (uvMode)
            {
                case UVMode.Clip: return new Vector2(uvs.x * uvScale.x, uvs.y * uvScale.y) - uvOffset;
                case UVMode.Clamp: return new Vector2(uvs.x * uvScale.x, (float)(uvs.y - clipFrom) / ((float)span) * uvScale.y) - uvOffset;
                case UVMode.UniformClip: return new Vector2(uvs.x * uvScale.x, (uvs.y+ uvBeginDistance) * uvScale.y) - uvOffset;
                case UVMode.UniformClamp: return new Vector2(uvs.x * uvScale.x, (uvs.y+ uvBeginDistance) * uvScale.y) - uvOffset;
            }
            return Vector2.zero;
        }
    }
