    ...
    public class ObjectController : SplineUser
    {
        ...
        protected override void Build()
        {
            ...
            for (int i = 0; i < spawned.Length; i++)
            {
                ...
                if (_applyRotation && (!_randomizeOffset || !_useRandomOffsetRotation)) spawned[i].rotation = evaluateResult.rotation;
                if (_applyScale) spawned[i].scale = spawned[i].baseScale * evaluateResult.size;
                ...
                if (_randomizeOffset)
                {
                    ...
                    if (_useRandomOffsetRotation) spawned[i].rotation = Quaternion.LookRotation(evaluateResult.direction, spawned[i].position - center);
                }
