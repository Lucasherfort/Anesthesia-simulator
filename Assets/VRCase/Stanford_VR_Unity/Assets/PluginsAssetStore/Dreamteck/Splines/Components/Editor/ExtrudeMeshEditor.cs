    ...
    public class ExtrudeMeshEditor : MeshGenEditor
    {
        ...
        public override void BaseGUI()
        {
            ...
            if (hasMeshes)
            {
                ...
                mirror = (ExtrudeMesh.MirrorMethod)EditorGUILayout.EnumPopup("Mirror", mirror, GUILayout.Width(100));
                EditorGUIUtility.labelWidth = 0;
                if (user.GetStartMeshMirror() != mirror) user.SetEndMeshMirror(mirror);
                EditorGUILayout.EndHorizontal();
            }
            ...
            user.extrudeUVMode = (ExtrudeMesh.ExtrudeUVMode)EditorGUILayout.EnumPopup("UV Mode", user.extrudeUVMode);
            if (user.extrudeUVMode != ExtrudeMesh.ExtrudeUVMode.Default)
            {
