...
namespace Dreamteck
{
    public static class ImageDB
    {
        ...
        {
            Texture2D image = Utils.LoadTexture(localDirectory, filename);
            if (image != null)
            {
                images.Add(image);
        ...
        public static void LoadImages(string localDirectory, string[] names)
        {
            ...
            {
                images.Add(Utils.LoadTexture("Spline/Editor/Icons", names[i]));
            }
        }

        public static void LoadImages(string localDirectory)
        {
            string path = Utils.FindFolder(Application.dataPath, "Dreamteck/" + localDirectory);
            ...
            for (int i =0; i < files.Count; i++) images.Add(Utils.LoadTexture(files[i]));
        }
