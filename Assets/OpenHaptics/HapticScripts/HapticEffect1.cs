...
public class HapticEffect1 : MonoBehaviour {
	...
	void Start () 
	{
		...
		devices = (HapticNeedleTest[])FindObjectsOfType(typeof(HapticNeedleTest));
	...
	void Update () 
	{
		...
		for( int ii = 0; ii < devices.Length; ii++ )
		{
            HapticNeedleTest device = devices [ii];
			...
			if (ID == -1)
			{
				FXID [ii] = HapticNeedleTest.effects_assignEffect(devices [ii].configName);
				ID = FXID [ii];
			...
			if (delta[ii]  <= Mathf.Epsilon)
			{
                ...
                HapticNeedleTest.effects_settings(
					device.configName,
					ID,
					Gain,
					Mag,
			...
			if (oldInTheZone != inTheZone [ii])
			{
				if (inTheZone [ii])
				{
                    HapticNeedleTest.effects_startEffect(device.configName, ID );
				} else
				{
                    HapticNeedleTest.effects_stopEffect(device.configName, ID);
				}
			}
...
#if UNITY_EDITOR
[CustomEditor(typeof(HapticEffect1))]
public class HapticEffect1Editor : Editor 
{
	override public void OnInspectorGUI()
	{
		HapticEffect1 HE = (HapticEffect1)target;
		...
		{
			HE.effectType = (HapticEffect1.EFFECT_TYPE)EditorGUILayout.EnumPopup("Effect Type", HE.effectType);


			switch (HE.effectType)
			{
			case HapticEffect1.EFFECT_TYPE.CONSTANT:
